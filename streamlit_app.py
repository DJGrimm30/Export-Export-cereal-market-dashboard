...
def load_eurostat_retail_sales_specific(filepath, specific_filters=None):
    ...
    try:
        ...
        # --- Original Robust Parsing (for complex Eurostat downloads that need melting) ---
        else:
            # st.write(f"Assuming {filepath} needs melting.") # Debug print removed
...
def load_eurostat_long_format_data(filepath, specific_filters=None):
    ...
    try:
        ...
        if specific_filters:
            ...
            if 'GEO' in df_melted.columns: df_melted = df_melted.rename(columns={'GEO': 'geo'})
            if 'UNIT' in df_melted.columns: df_melted = df_melted.rename(columns={'UNIT': 'unit'})
            if 'FREQ' in df_melted.columns: df_melted = df_melted.rename(columns={'FREQ': 'freq'})
            if 'COFOG_L2' in df_melted.columns: df_melted = df_melted.rename(columns={'COFOG_L2': 'cofog_l2'})
            if 'IND_TYPE' in df_melted.columns: df_melted = df_melted.rename(columns={'IND_TYPE': 'ind_type'})
            if 'AGE' in df_melted.columns: df_melted = df_melted.rename(columns={'AGE': 'age'})
            if 'SEX' in df_melted.columns: df_melted = df_melted.rename(columns={'SEX': 'sex'})
            ...
            required_final_cols = ['date', 'geo', 'value']
            if 'geo' not in df_melted.columns: st.warning(f"Final 'geo' column missing after processing {filepath}. Defaulting to 'Unknown'."); df_melted['geo'] = 'Unknown' 
            if 'value' not in df_melted.columns: st.warning(f"Final 'value' column missing after processing {filepath}. Defaulting to 0."); df_melted['value'] = 0 
