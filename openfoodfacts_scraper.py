...
st.set_page_config(
    ...
    # Theme will be applied via .streamlit/config.toml
)

# --- Custom CSS for Card-like UI ---
st.markdown("""
<style>
    ...
    /* Specific styling for the first "Analytics" card (mimicking gradient) */
    .gradient-card {
        background: linear-gradient(135deg, #FF6B6B, #FFA07A); /* Vibrant gradient */
        color: white; /* White text for gradient card */
        padding: 20px;
        border-radius: 12px;
    ...
    .st-emotion-cache-vk330y { /* Target sidebar container */
        ...
        color: white;
        border-radius: 0 12px 12px 0; /* Rounded right corner for sidebar */
    }
    .st-emotion-cache-1jm15vj { /* Sidebar header */
        color: white;
    }
    .st-emotion-cache-1w04006 { /* Sidebar multiselect labels */
        color: white;
    }
    ...
    /* Input fields */
    .st-emotion-cache-1c7y2kl { /* Text input */
        border-radius: 8px;
...
st.title("📊 Analytics Dashboard") # Changed title to match image
st.markdown("---") 
...
def load_eurostat_data(filepath, geo_col='geo', value_col='value', date_col='date', specific_filters=None):
    ...
    try:
            ...
            if specific_filters:
                for col_filter_name, filter_value in specific_filters.items():
                    ...
                    for col in df_processed.columns:
                        if col_filter_name.upper() == col.upper():
        ...
        else: # Assume it's already flat or simple
            df_processed = df.copy()
